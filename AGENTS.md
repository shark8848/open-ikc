...
1. 统一日志文件 `docs/worklog.md`（流水日志，按日期追加），另设 **`process.md`（根目录，当前未完成事项待办清单）**：worklog 记「发生了什么」，process.md 记「还欠什么」。
...
### 8.3 process.md 待办清单契约（强制）

> **开始任务前必须读取 `process.md`；完成任务时/收尾必须更新 `process.md`。** 未完成内容禁止只停留在对话或提交信息里，必须以 process.md 落盘。

1. **任务开始前**：先读 `process.md` + `docs/worklog.md` 最近条目，继承未完成项与上下文；若本次工作命中 process.md 中某待办，先把该行状态改为「进行中」。
2. **任务进行中**：新发现的未完成项（环境遗留、待决策、缺口）及时登记到 process.md（含状态/优先级/下一步），避免遗忘。
3. **任务完成/收尾时**：更新 process.md——已闭环的项删除或移入 worklog 完成记录，剩余项刷新状态；**未闭环事项必须保留在 process.md**。
4. 同步提交：process.md 与 worklog 随任务改动一起 commit + push（遵循 §8.2）。
5. 与 worklog 的关系：worklog 是流水与决策记录，process.md 是当前待办清单；两者相互引用，冲突时以 `AGENTS.md` 为准。
...
6. `process.md`（当前未完成事项清单）与 `docs/worklog.md`（流水日志）——任务开始先读、完成时更新
7. V1 / 规划类长文仅作背景参考，不直接当接口真理
...
| 未完成待办 | `process.md`（任务开始先读、完成时更新） |
| 流水日志 | `docs/worklog.md` |
