...
- **命令/工具白名单（只读）**：CLI 为 `kb-list`/`kb-get`/`doc-get`/`parse-query`/`sys-catalog`/`sys-error-codes`/`search-query`/`deep-search`；MCP 为 `sys_catalog`/`sys_error_codes`/`kb_get`/`kb_query`/`doc_get`/`parse_query`/`parse_issue_ticket`/`search_query`/`deep_search`。禁止任意 shell 与写操作。
