...
2. **主库形态**：~~SQLite（单机）还是 PostgreSQL（多实例/热备）？~~ **已定稿（2026-09-15，见 `docs/开放平台研发计划_v1.0.md` §4.1）**：按环境分工——SQLite 只用于本地开发与单元测试（`tmp_path` 隔离），PostgreSQL 16 用于集成测试、联调、预发与生产；两环境共用同一套 SQLAlchemy 模型与 Alembic 迁移，禁止业务代码方言分支；管理面库（`OPEN_PLATFORM_DB_PATH`）继续保留 SQLite。
