...
### 补记：origin 网络恢复，双远端同步（2026-09-05）

- 用户确认网络恢复后，执行 `git push origin main`：`726a133..a3b1450` 推送成功；github 此前已推送（`7542e5c..a3b1450`）。本地 main 与双远端均同步到 a3b1450。
- process.md 中「origin 网络不可达」环境遗留项闭环。
- 未纳入提交的用户并发改动（app/、SDK 等）仍在工作区。
...
### 任务：架构总图治理运营与能力层扩展（2026-09-06）

- 治理与运营条：新增「配置管理」分组（配置中心：环境变量/运行参数集中管理、模型路由/限流阈值/开关）。
- 运营分析：增加知识运营条目「知识热度 / 反馈 · 迭代闭环」。
- 能力层：原「文档采集器」列拆分为上下两个 Channel——文档采集器（外部文档/网页/知识库统一适配）+ 新增「外部检索 Channel」（Web / 企业搜索 / 统一适配）。

### 任务：架构总图调度层换色（2026-09-06）

- 调度层由琥珀黄改为靛蓝（gIndigo #6366f1→#4338ca）：层带底色 #eef2ff/#c7d2fe、Redis 描边与层内箭头 #3730a3、副标题 #e0e7ff；与全图冷色系（蓝/青/紫）协调，避免黄色突兀。

### 任务：架构总图 openikc sdk 风格统一（2026-09-06）

- 接入层 openikc sdk 框改为与 ikc-sdk-lib 组件条一致的风格：粉底 #fdf2f8、粉色虚线描边 #ec4899、标题 #be185d、副标题 #9d174d，体现二者同为 SDK 体系。
...
### 任务：架构总图外部通道组件去描边（2026-09-06）

- 「文档采集器」「外部检索 Channel」两框取消 #047857 描边，仅保留翡翠绿渐变填充，与能力层其余无描边组件风格一致。

### 任务：架构总图版本号更新（2026-09-06）

- 图内标题下方版本标识 v1.0 → v6.0（与企业级智能知识库 V6.5 规划呼应）；文件名暂保持不变，避免引用断裂，如需随版本改名待确认。

### 任务：架构总图 Redis 队列与 Parser Cache 去描边（2026-09-06）

- 「Redis 队列」取消靛蓝描边 #3730a3、「Parser Cache」取消青色描边 #0e7490，均仅保留渐变填充，与其他无描边组件风格统一。

### 任务：架构总图知识文档类型补全（2026-09-06）

- 数据层「知识文档」卡片格式列表补全：PDF / Word / MD / PPT、Excel / HTML、音视频 / 图片、Wiki 页面（加工产物），与服务引擎层解析引擎支持的格式对齐；行距微调适配框高。

### 任务：架构总图 LLM Gateway 职责调整（2026-09-06）

- LLM Gateway 副标题移除「Prompt 管理」，保留「统一大模型网关 · 配额路由」。

## 2026-09-15

### 任务：基于架构总图输出全平台研发计划 v1.0
...
- 隐性补齐（图中未显性、但已由 process.md / 数据模型方案 / 全局契约记为缺口）：限流与配额、PipelineRun 任务对象、配置中心、评测、内容合规审查、文档采集器连接器、外部检索 Channel、JS/TS SDK、SDK 制品发布链路。
- 四个专项表：① SDK 与接入形态（16 项，含 `open-ikc-sdk` Python/Java、MCP 26 工具、CLI 26 子命令、`ikc-sdk-lib`、`ikc_core_sdk`、`openwiki-server-sdk`、待建 `semantica-sdk` / TS SDK、`pyuploadx`、`litellm` 封装、log-center 三语言）；② 数据结构设计（12 行，对齐数据模型方案 ①–⑥ 概念层 + 加工执行域 + 治理支撑域与 M0–M3）；③ 接口设计（11 行，四类能力 + wiki/graph + 索引待开放 + 系统/管理面 + 内部服务间 + 契约 G0–G11）；④ 对接与联调（14 行，对端含 ikc-core-service 19000、openwiki-server、semantica、retriever、不同解析引擎、PyUploadX、LLM 网关/模型服务、log-center、IdP、MinIO、PostgreSQL、图库、CI E2E）。
...
- 重写 `docs/开放平台研发计划_v1.0.md` §2：由「一行一组件、单元格堆多条」改为**一行一条明细**，`明细类型` 分 `组件` / `SDK` / `引擎能力` / `研发任务` 四类，编号 `L0-01`…`X-41`，共 **260 条**（组件 40 / SDK 44 / 引擎能力 48 / 研发任务 128）。
- 分层行数：L0 26、L1 39、L2 22、L3 65、L4 33、L5 34、横切 41；文首新增「明细行数统计」表。
- 重写 §5 接口设计为**一行一个接口**（`I-01`–`I-54`，共 54 条）：知识库 4 / Wiki 5 / Graph 5 / 文档 5 / 解析 5 / 检索 3 / 索引 2 / 系统面 6 / 管理面 11 / 内部服务间 7 / 跨层契约 1，并保留「分组设计要点」汇总表。
...
### 任务：研发计划移除本体引擎（不在项目范围）

- 修改 `docs/开放平台研发计划_v1.1.md`：删除 §1 L3 总览中的「本体引擎 `ontolith` / `ontolith-sdk`」、§2.4 三条明细（组件 / SDK / 研发任务）、§3 SDK 表中的 `ontolith-sdk` 行、§10 组件索引中的「本体引擎」行；阶段出口口径同步去掉「本体服务面」。
- 统计更新：§2 明细由 180 条调整为 177 条（组件 37 / SDK 31 / 引擎能力 38 / 研发任务 71），L3 由 43 条调整为 40 条。
- 同步：`docs/worklog.md` 2026-09-15 早前条目与 `process.md` 历史条目的枚举表述一并去掉本体引擎。
- 说明：仅文档表述调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：研发计划修正 Task 调度器现状（已在 GitLab 其它工程实现）

- 修改 `docs/开放平台研发计划_v1.1.md` §1 L2 总览与 §2.3 明细：`Task 调度器` 现状由「部分落地 / 待落地」修正为「**已实现（GitLab 仓库其它工程）**」，平台与实例侧为对接复用，不再重复建设调度与 worker。
- 逐条调整：`L2-04` 组件（已实现，外部工程）、`L2-05` SDK（补调度器客户端）、`L2-06`（改为平台侧对接既有调度器）、`L2-07`（`PipelineRun` 以既有任务模型为准，平台只做映射）、`L2-08`（任务 ID 贯通 平台 → 调度器 → 引擎）、`L2-12`（编排与调度职责边界）。
- 新增链路：§5.5 内部服务间 `I-66`（平台 → `Task 调度器`）与 §6 对接联调「`Task 调度器`（GitLab 其它工程，已实现）」一行。
- 说明：仅文档表述调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：研发计划去除「同上 / 同组件」简写，逐行写明具体内容

- 修改 `docs/开放平台研发计划_v1.1.md`：§2 分层明细中 140 处「同上」（子系统 / 组件列）全部展开为具体组件名（如 `ikc open platform portal`（平台仓 `portal/`）、`ikc-core-service`、`Task 调度器`…）；61 处「同组件」（交付物 / 验收列）全部展开为该组件的具体验收内容。
- §4.1 环境矩阵「联调 / 预发 / 生产」行由「同上（独立实例与账号…）」改为显式写出 `OPEN_PLATFORM_DB_DSN=postgresql+psycopg://<user>:***@<host>:5432/<db>` 与说明。
- §2 读法说明同步改写（不再提及「同组件」约定）。
- 校验：全文已无「同上 / 同组件」；表格列数一致（§2 明细 177 条、§5 接口 90 条、§6 对接 16 条），文档 524 行。
- 说明：仅文档表述调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：研发计划表格按层 / 组件 / 引擎加色标区分

- 修改 `docs/开放平台研发计划_v1.1.md`：全部 21 张表按维度加 emoji 色标，色块落在**首个数据列**，宽表同时落在**分类列**（`子系统 / 组件` / `分组` / `链路`，共 236 处镜像），每张表前插入 `> 色标（…）：…` 图例行。
- 着色维度：§1 总览与明细统计→按层；§2.1–§2.7 分层明细→按组件 / 引擎；§3 SDK 表→按归属层；§4.1→按环境（本地 / 单测 SQLite、集测 / 生产 PostgreSQL）；§4.2→按数据域；§5.1–§5.4→按接口分组；§5.5→按内部链路；§6→按对端类别；§7→按阶段；§8→按优先级；§10→按平台自身 / 引擎契约 / 日志中心。
- 文档头新增「色标约定」说明：选用 emoji 色块而非单元格底色，原因是 GitHub 等渲染器会过滤 HTML `style` / `bgcolor`；色块与渲染器无关（GitHub / IDE 预览 / 终端均可见）。
- 修正图例块与表格之间缺空行的问题（21 处）：`> 色标…` 紧邻表格行会被解析为 blockquote 的 lazy continuation，存在表格不渲染的风险，现已统一补空行。
- 校验：全部表格行列数一致（§2.1–§2.7 均为 9 列、§3 9 列、§4.1 6 列、§4.2 8 列、§5 8 列、§6 8 列、§7 5 列、§8 3 列、§10 3 列）；§2 明细 177 条、§5 接口 90 条、§6 对接 16 条；文档 568 行。
- 说明：仅文档表述调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：研发计划文档头重构为「0. 文档说明」章节

- 修改 `docs/开放平台研发计划_v1.1.md`：原标题下堆积的 6 行引用块（平台主体仓库 / 输入依据 / 时间口径 / 优先级 / 表格约定 / 色标约定）拆解为 `## 0. 文档说明` 三张表——`0.1 口径与约定`（6 行）、`0.2 输入依据`（5 行：架构图 / 平台自有文档 / 依赖 SDK 与版本 / 数据与契约方案 / 引擎侧资料）、`0.3 阅读路径`（9 行：想了解 × 直接看）。
- 「为何用 emoji 而非单元格底色」单独成行，不再与色标描述挤在一条里。
...
### 任务：Task 调度器 只表达「服务已实现 + SDK 已实现」

- 修改 `docs/开放平台研发计划_v1.1.md`：删除全部「（已在 GitLab 仓库其它工程实现）/ 外部工程 / 既有调度器」口径，`Task 调度器` 统一表达为**服务已实现 + SDK 已实现**。
- §1 L2 概述：组件列改为「`Task 调度器`（服务 + SDK 已实现）」；SDK 列去掉「**待建** 统一 `TaskClient`」，改为「`Task 调度器` 客户端 SDK（已实现）」；现状列改为「`Task 调度器` 服务与 SDK 已实现；平台侧异步提交与轮询（`executeMode=async`）已落地」。
- §2.3 明细：`L2-04` 组件行去掉「位于 GitLab 仓库其它工程…不再重复建设」与现状「已实现（外部工程）」→「已实现」；`L2-05` SDK 行现状「已有」→「已实现」；删除 3 条 `Task 调度器` 研发任务行（平台侧对接 / 任务对象口径对齐 `PipelineRun` / 任务 ID 贯通），该表只保留服务与 SDK 两条。
- §2.3 重新编号：原 `L2-09`–`L2-18` 收敛为 `L2-06`–`L2-15`；`L2-09`（原 Pipeline 编排职责边界行）改写为「`Pipeline 编排` 管状态机与阶段回调，`Task 调度器` 管排队 · 重试 · 优先级」，不再表述为对接外部工程。
- §5.5：`I-66` 链路名去掉「（GitLab 其它工程）」，现状「已实现（外部工程）」→「已实现」，设计任务去掉「不重复建设」。§6 对接联调对端名改为「`Task 调度器`（服务 + SDK 已实现）」。
- 统计更新：§2 明细 177 → 174 条（L2 由 18 → 15：组件 5 / SDK 3 / 引擎能力 1 / 研发任务 6），研发任务合计 71 → 68，§2「明细行数统计（共 174 条）」与 §1 统计表同步。
- 校验：全文无 GitLab / 外部工程 / 既有调度器 残留；表格列数全部一致（§2.3 由 20 行块变为 17 行块）。
- 说明：仅文档表述调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：L4 模型服务改为「只登记需要、不排期研发」

- 修改 `docs/开放平台研发计划_v1.1.md` §2.5：`模型服务`（LLM / vLLM / Embedding / Ranking / ASR）4 行重构为 3 行——`L4-05` 需求（能力需要：LLM 推理 / Embedding 向量化 / Ranking 重排 / ASR 转写，现状=既有服务、工程已部分直接调用）、`L4-06` SDK（OpenAI 兼容客户端 + ASR 客户端，已有）、`L4-07` 需求（平台侧接入需要：四类业务统一经 `LLM Gateway`，模型服务本身不在本计划研发范围）；删除原 `L4-08` 研发任务行（部署与容量规划 / 健康检查与灰度 / 模型注册与版本管理）。
- 三行 `优先级` = 依赖、`阶段` = 非研发、`时间安排` = 不排期，交付物改为「调用清单（服务地址 · 模型名 · 鉴权 · 限流口径）」「客户端已可用，工程直接调用」「接入路径统一走网关」。
- 新增明细类型 `需求`：§0.1 行粒度与 §2 读法均补 `需求` 并说明其含义（只登记能力需要、不排期研发）。
- §2.5 重新编号 `L4-05`–`L4-17`；§2.5 色标图例改为「🟩 `模型服务`（既有服务 · 只登记需要）」。
- §1 L4 概述同步：组件列标注「既有服务，只登记需要」，引擎能力列去掉「推理与向量化 / 重排 / ASR」（由既有模型服务提供，非平台研发），现状列补「模型服务为既有服务（工程已部分直接调用）」。
- 统计表新增「需求」列：L4 3 / 4 / 3 / 5 / 2 = 17；合计 36 / 31 / 37 / 67 / 2 = **173**（§2 标题与 §1 统计表同步）。
- §5.5 `I-65` 现状「未接入」→「部分（模型服务已可用，网关未落地）」；§6 对接联调对端改为「LLM 网关（平台侧新建）· 模型服务（既有：LLM / vLLM / Embedding / Ranking / ASR）」。
- 校验：表格列数全部一致（§1 统计表 7 列；§2.5 块 19 行）；文档 610 行。
- 说明：仅文档表述调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：把开放平台架构总图嵌入研发计划文档

- 修改 `docs/开放平台研发计划_v1.1.md`：在 §1 分层总览标题下嵌入 `![开放平台架构总图 v1.0（六层 + 横切治理与运营）](images/开放平台架构总图_v1.0.svg)`，并加一行说明「输入依据：`images/开放平台架构总图_v1.0.svg`（图内 v6.0），即上图；§1 总览表与 §2 各层明细均按图中的分层与组件逐条展开」。
- 相对路径与文档同目录树（`docs/images/…`，图文件已在库中，GitHub 可直接渲染 SVG）。
- 配套：§0.2 输入依据「架构图」行标注「已嵌入 §1」；§0.3 阅读路径新增「平台架构总图（六层 + 横切）→ §1 开头」。
- 校验：文档图片语法唯一且路径存在（24212 B）；26 张表列数未变；文档 606 行。
- 说明：仅文档结构调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：§3 SDK 专项补入引擎族 SDK 计划

- 修改 `docs/开放平台研发计划_v1.1.md` §3：表格由 14 行扩到 20 行，新增引擎族 / 调度 / 模型的 SDK 行——解析引擎族 SDK（建议坐标 `ikc-parse-sdk`，P1 阶段一）、检索客户端 SDK（建议坐标 `ikc-retriever-sdk`，P1 阶段一）、文档采集器连接器 SDK（建议坐标 `ikc-connector-sdk`，P2 阶段二）、外部检索 Channel 适配 SDK（P3 阶段二）、`Task 调度器` 客户端 SDK（已实现，平台侧薄封装 P1 阶段一）、模型服务客户端（OpenAI 兼容 / ASR，沿用 §2.5 口径：只登记需要、不排期研发）。
- §3 表内按归属重排为「L1 平台 SDK 与契约 → L0 待建 Java/TS → L3 引擎族 SDK（实例 / Wiki / 图谱 / 解析 / 检索 / 采集 / 外部检索）→ L2 调度 → L4 能力 → 横切日志」；图例同步为「🟩 L1 接入 / L2 调度」。
- 新增 `### 3.1 引擎族 SDK 统一约定`（命名 / 鉴权 / 链路 / 错误 / 超时与重试 / 产物 / 交付物，7 行），避免业务层为每个引擎写一套适配。
- 同步 §2.4：补 `L3-40` 外部检索 Channel 的 SDK 行（外部检索适配 SDK 待建，P2 阶段二），§2.4 重新编号至 `L3-41`。
- 统计更新：L3 SDK 7 → 8、L3 小计 40 → 41，合计 SDK 31 → 32、总计 173 → **174**（§1 统计表、§2 标题与 §1 L3 概述同步）。
...
### 任务：核对架构总图 v1.1 的 SDK 与计划覆盖度并补齐缺口

- 逐项核对 v1.1 图内 12 处 SDK（10 处内部 SDK + 接入层对外平台 SDK + 跨层 `ikc-sdk-lib`）与 §2 明细 / §3 专表：名称 100% 有对应条目，但 4 处**内容口径**不齐，已修：
  - `ikc-sdk-lib` 在 §2.2 只有组件 / 研发任务行、无 SDK 行 → 新增 `L1-35` SDK 行（业务模型 · 枚举 · traceId 与错误码协议封装）。
  - `Universal Retriever` 的 §2.4 SDK 行只写 `kb_search`（无客户端 SDK）→ 拆为 `L3-20`（`kb_search` 索引 schema 库）+ 新增 `L3-21`（`ikc-retriever-sdk` 待建：检索客户端 + 结果模型，P1 阶段一）。
  - `Parse Engines` SDK 行未写名称 → 明确为 `ikc-parse-sdk`（待建）；`文档采集器` SDK 行原混写「`pyuploadx`（上传前置）+ 连接器 SDK」→ 改为 `ikc-connector-sdk`（待建），pyuploadx 由 §2.5 `L4-12` 承载。
- §2.2 / §2.4 重新编号（L1 至 `L1-36`、L3 至 `L3-41`）；§1 L3 概述 SDK 单元改为「已实现 2 个 + 待建 5 个」的显式清单。
- 新增 `§3.2 内部 SDK 层对照（对应架构总图 v1.1）`：12 行逐项给出「架构图位置 / 组件 · 引擎 / SDK / 状态 / 计划条目」，并给覆盖结论（已实现·已发布 6 处、待建 6 处）。
- 统计更新：L1 SDK 5 → 6（小计 35 → 36）、L3 SDK 8 → 9（小计 41 → 42）、SDK 合计 32 → **34**、总计 174 → **176**；§0.3 阅读路径 SDK 行补「与架构图逐项对照 §3.2」。
- 校验：28 张表列数一致（§3 23 行 9 列、§3.1 9 行 2 列、§3.2 14 行 5 列）；文档 652 行。
- 说明：仅文档表述调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：SDK 计划深挖（方法级清单 + 交付版本）+ 版本维护章节重写

- `docs/开放平台研发计划_v1.1.md` §3.2：新增「交付版本」「交付窗口」两列（12 行逐项给出 1.0.0 / 0.1.0 新建 / 0.1.0 → 0.2.0 / 0.6.1 → 0.7.0 等版本与所属阶段 + `__/__ – __/__` 窗口）。
- 新增 `§3.3 内部 SDK 方法级清单`（38 行，一行一个能力面）：对 10 个内部 SDK + 对外平台 SDK 给出「方法 / 能力面 → 入参要点 → 出参 / 结果模型 → 状态」，冻结 `TaskHandle` / `TaskStatus` / `Hit{...}` / `FusedHit` / `ParseTask` / `ParseStatus` / `ExternalHit` / 缓存 `stats` 等结果模型。
- `§11 版本与维护` 重写为三小节：11.1 文档版本（v1.0 / v1.1 / v1.1+r1 色标与文档说明 / v1.1+r2 调度器与模型服务口径 + 架构图 v1.1 / v1.1+r3 本轮 SDK 深挖）、11.2 配套资产版本（架构总图 v1.1、研发计划 v1.1+r3、数据模型方案 v2、全局契约 v0.3、process.md、worklog）、11.3 维护约定（阶段结束 / 架构图变更 / 接口变更 / SDK 发布 / 错误码变更 / 文档口径变更 各自的同步清单）。
- §0.3 阅读路径补「文档版本、配套资产与维护约定 → §11」。
- 修复：加列脚本误伤 §7 阶段表两行（阶段二 / 跨阶段被插入多余列），已还原为 5 列。
- 校验：32 张表列数全部一致（§3.2 14 行 7 列、§3.3 40 行 5 列、§11.1/11.3 各 3 列）；文档 726 行；同步 `process.md` 待评审条目计数。
- 说明：仅文档表述调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：架构总图升级为 v1.2（本体组件 + UR 检索范式标注）

- 产出 `docs/images/开放平台架构总图_v1.2.svg`（1900×1575，341 行；由 v1.1 复制改造，v1.1 保留不动）。
- 新增**本体构建引擎**（服务引擎层，四列重排为五列，等宽 232px：core / openwiki / graph / 本体 / Universal Retriever），琥珀色渐变 `gAmber`（#f59e0b→#d97706）+ 深琥珀描边，职责=本体建模 / 概念抽取 / Schema 演化。
- 新增**本体存储**（数据层存储设施第 4 框，Ontology Store：OWL / RDF / SKOS · 本体 Schema · 概念词典），琥珀色浅底 #fef3c7 + 琥珀描边，存储设施内框由 3→4（等宽 136px）。
- Universal Retriever 内新增 4 个检索范式 chip：RAG / GraphRAG / OAG / Agentic RAG（白底玫红描边），副标题改「通用 / 融合检索引擎」。
- 本体相关统一琥珀色区别标识：构建引擎、本体存储、`ikc-ontology-sdk`（内部 SDK 待建，琥珀虚线 chip）+ 琥珀虚线箭头「本体模型落库」（构建引擎 → 本体存储，经能力层 Parser Cache / pyuploadX 间隙走线）。
- 版面联动：服务引擎层调度入箭头 3→4、能力调用箭头 4→5（列心 316/563/810/1020/1304）；UR 同步调用路径终点 x 1275→1304；「索引」资产补「本体索引」；底部图例新增琥珀色说明（本体相关组件）与内部 SDK 覆盖清单（含本体构建引擎）。
- 校验：XML 解析通过（viewBox 0 0 1900 1575），本体构建引擎 ×4 / 本体存储 ×3 / ikc-ontology-sdk ×2 / 四范式 chip 齐备；cairosvg 本机不可用跳过渲染（与 v1.1 轮一致）。
- 说明：仅文档配图调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：架构总图 v1.2 修订（本体组件淡琥珀无边框 + 顶部三类角色图标）

- 本体组件配色减淡并去描边：`gAmber` 改淡琥珀渐变（#fde68a→#fcd34d）；本体构建引擎 / 本体存储 / `ikc-ontology-sdk` chip 全部去除描边（无边框模式，与能力层外部通道风格对齐），文字改深琥珀 #92400e / #a16207；「本体模型落库」连线与箭头柔化为 #ca8a04；底部图例说明同步。
- 画布扩至 1900×1665：主体六层内容整体下移 90px（`<g transform="translate(0, 90)">` 包裹，坐标零重算）；顶部新增「平台角色」条（y=64–140，左侧竖排标签与层名样式一致）。
- 新增三个形象角色卡（图标 + 标题 + 职责描述，虚线指向应用层）：知识开发生产（蓝 #3b82f6，人形 + 齿轮图标，领域专家/知识工程师）、知识使用者（青 #14b8a6，人形 + 放大镜图标，业务用户/智能问答前端）、知识运营者（绿 #10b981，人形 + 柱状图图标，运营/评测/治理）。
- 修复：底部图例第二段（内部 SDK 覆盖）起始 x 560→645，消除与琥珀色说明的文字重叠。
- 校验：XML 解析通过（viewBox 0 0 1900 1665），三角色卡 / 淡琥珀 / 无边框 / translate 组齐备，#b45309 残留 0；本轮 SearchReplace 对该文件出现多次匹配异常，改用 sed 完成单点 x 坐标修正。
- 说明：仅文档配图调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：架构总图 v1.2.1（去版本行 + 标题间距）+ 16:9 PPT 版

- 产出 `docs/images/开放平台架构总图_v1.2.1.svg`（1900×1700）：整行去除「v1.2 · …」版本说明（版本仅保留在文件名）；标题与角色条间距加大——角色条下移 26px（`<g transform="translate(0, 26)">` 包裹）、主体六层 translate 90→126（画布 1665→1700），标题→角色条间距约 60px、角色条→应用层间距 6px，角色虚线箭头端点同步 151→160。
- 产出 `docs/images/开放平台架构总图_v1.2.1_PPT.svg`（1920×1080，16:9）：基于 v1.2.1 全量内容（角色条 + 六层 + 双竖条 + 图例）以 `scale(0.60)` 整体缩放嵌入，内容底部 1068 ≤ 1080 无溢出；标题改为原生字号 28（y=38）不随缩放，内容水平居中（translate x=390）。
- 校验：两文件 XML 解析通过，`<g>` 开闭配平（36/36、37/37），版本行 0 残留。
- 说明：仅文档配图调整，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：新增分层组件清单 Excel（v1.2.1）

- 产出 `docs/开放平台分层组件清单_v1.2.1.xlsx`（181 行 × 10 列，两个 sheet）：
  - **分层组件清单**：编号 / 层 / 模块·组件·引擎·功能 / 类型 / 功能描述 / 现状 / 进展 / 归属（开发·使用·运营）/ 时间安排 / 交付物·验收；数据=研发计划 v1.1 §2 全部 176 条明细（L0×18 / L1×36 / L2×15 / L3×42 / L4×17 / L5×25 / 横切×23）+ 架构总图 v1.2.1 新增 5 行（V1.2-01~05：本体构建引擎 / ikc-ontology-sdk / UR 四检索范式 RAG·GraphRAG·OAG·AgenticRAG / 本体存储 / 落库链路，琥珀色高亮标记）。
  - **分层汇总**：各层条目数与进展分布（合计 181：已完成 68 / 进行中·持续 12 / 未开始·待规划 95 / 既有·非研发 6）+ 口径说明。
  - 口径：进展由「现状」映射（已落地/已实现/已发布→已完成；部分/WIP/进行中→进行中；待/未→未开始；既有/外部→非研发）；归属=主要服务角色（Portal 与横切治理→运营，3rd App 及接入 SDK/指南→使用，其余平台建设→开发）；时间安排原文为空待排，以「阶段 · 优先级（时间待排）」表达。
  - 样式：层列按架构图配色浅底、进展列红黄绿标注、V1.2 新增行琥珀底；冻结首行 + 自动筛选。
- 说明：仅文档配套清单，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：清单 Excel 新增「workhub 对接计划」sheet

- `docs/开放平台分层组件清单_v1.2.1.xlsx` 追加第三个 sheet「workhub 对接计划」（19 项 × 7 列）：从数字员工类平台应用（workhub）视角列出对接清单，覆盖六类——准入准备（应用注册/API Key、数字员工身份与 AUTHZ 对接（X-Auth-System=digital_employee）、限流配额）、平台机制（统一响应壳、Trace 头透传）、接入形态（Python SDK / MCP ikc-op-mcp / CLI / Java SDK 规划 / 指南与示例）、知识库能力（创建/更新/查询）、文档与解析（ingest 异步接入、parse + 结果下载票据、文档查询）、检索能力（universal-search / deep-search / 兼容别名 / GraphRAG·OAG 规划）。
- 每行含优先级（P0×9 / P1×5 / P2×4 / P3×1）、依赖现状与「清单引用」——指向 Sheet1「分层组件清单」的编号 + 行号（如 L1-14 第 33 行、V1.2-03 第 180 行），行号随 Sheet1 数据自动映射，全部解析无缺失。
- 样式与既有 sheet 一致：类别列六色底、优先级红黄标注、冻结首行、底部口径说明。
- 说明：仅文档配套清单，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：新增知识加工转换全景图 v1.0

- `docs/images/知识加工转换全景图_v1.0.svg`（1900×830）：以「图标 + 文字 + 过程箭头」全景展示知识从文档到元数据、索引、Wiki、图谱、本体的转换关系。六阶段横向流程：① 知识源（PDF/Word/Excel/PPT/MD/HTML/音视频/图片 + 外部源）→ ② 接入与登记（采集器 / pyuploadX / MinIO·S3 / 元数据 PostgreSQL）→ ③ 解析与产物（解析引擎族 / Parser Cache / content_template）→ ④ 加工转换四路并行（索引化→RAG、Wiki 化、图谱化→GraphRAG、本体化→OAG，每路标注 过程：切片→Embedding→索引 等转换步骤）→ ⑤ 知识资产（索引 / Wiki 页面 / 图谱三元组 / 本体存储）→ ⑥ 检索与消费（Universal Retriever 四范式 chips → 数字员工·3rd App）。
- 横切层：治理与运营（知识生命周期 / 质量评测 / 血缘与追溯），绿色虚线上指反馈箭头；注记「元数据贯穿 ①→⑥ 全流程」。
- 配色与架构总图 v1.2.1 一致（本体化与本体存储淡琥珀无边框）；边标签加白色 halo 防交叉遮挡；cairosvg 渲染验证布局（CJK 方块仅为渲染器缺字体，浏览器正常）。
- 仅新增配图，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：知识加工转换全景图 v1.1（规则/模型驱动小图标）

- `docs/images/知识加工转换全景图_v1.1.svg`（基于 v1.0 复制优化，1900×830）：为各转换步骤增加驱动方式小图标——`icoRule`（剪贴板核对，灰 #334155）= 规则驱动、`icoModel`（神经网络，紫 #7c3aed）= 模型驱动，defs 复用 + `<use>` 引用。
- 标注位置：② 元数据登记（规则：属性/哈希/状态登记；模型：摘要增强）；③ 解析引擎族（模型：版面/ASR/LLM 抽取；规则：表格/模板清洗）；④ 四路加工各加驱动行——索引化（规则：切片/索引构建/增量调度；模型：Embedding 向量化）、Wiki 化（模型：词条生成 LLM；规则：页面树/互链/版本管理）、图谱化（模型：实体/关系抽取/消解；规则：词典模式/入库写边）、本体化（模型：概念抽象 LLM；规则：Schema 演化/词典/人工评审）。
- 图例新增第二行「规则驱动 / 模型驱动」标识说明；按用户要求删除图中「与架构总图 v1.2.1 一致」表述（副标题改为「各转换步骤以小图标标注驱动方式：规则 / 模型」，本体化说明行移除该引用）。
- cairosvg 渲染验证布局正常；仅新增配图，未改代码行为，跳过 Claude 只读审查（AGENTS.md §13.1）。

### 任务：知识加工转换全景图 v1.1 微调（解析引擎 + 连线标签）

- ③ 文档解析引擎族：版面改为「paddle ocr / mineru」（替换 marker/docling 表述）；过程行增加「抽取 → 清洗 → 去重」；驱动行规则项同步改为「清洗 / 去重 / 模板」。
- ⑤ 知识资产 → ⑥ Universal Retriever 四条连线上删除检索方式标签（RAG / GraphRAG / OAG / Agentic）——检索范式仅在 UR 内 chips 表达；UR → 数字员工连线的「融合检索（答案 · 引用）」保留。
- cairosvg 渲染验证正常；仅配图微调，跳过 Claude 只读审查。

### 任务：知识加工转换全景图 v1.1 微调（知识源图标 + 删底部小字）

- 删除全景图底部「※ 元数据贯穿 ①→⑥ 全流程…」注释小字行（元数据贯穿语义仍由治理条「血缘与追溯」承载）。
- ① 知识源 8 类文档卡片各配代表性彩色小图标：PDF（红页脚 PDF）/ Word（蓝页 W）/ Excel（绿表格网格）/ PPT（橙投影屏+柱状）/ MD（M↓ 框）/ HTML（</> 括号）/ 音视频（紫播放圆钮+声波）/ 图片（青相框+山形太阳）；替换原统一灰色文档图标。
- cairosvg 渲染验证正常；仅配图微调，跳过 Claude 只读审查。

### 任务：新增知识应用场景全景图 v1.0

- `docs/images/知识应用场景全景图_v1.0.svg`（1900×1020）：不同类型知识应用的场景图。中心为「知识服务平台」（玫瑰红，RAG / GraphRAG / OAG / Agentic 四范式 chips + API/SDK/MCP/CLI 接入 + Bearer 认证 / traceId / 权限过滤），左右各三张场景卡（双向箭头连接）：
  - 左列：问答咨询型（RAG：智能客服 / 制度政策问答 / FAQ 助手）、门户浏览型（Wiki：知识门户 / 帮助中心 / 词条互链）、学习赋能型（RAG+Wiki：培训路径 / 知识推送 / 考核陪练）；
  - 右列：任务执行型（Agentic RAG：数字员工办事 / 工单处理 / 报告生成）、分析推理型（GraphRAG：风险传导 / 根因定位 / 关系挖掘）、语义治理型（OAG·本体：智能标注 / 跨域检索 / 概念对齐，淡琥珀虚线）。
  - 每卡含类型图标 + 范式 chip + 3 个场景 chip + 典型用户 + 技术说明；底部「知识运营场景」绿虚线条（知识加工与发布 / 质量评测 / 生命周期管理 / 运营分析），与平台双向箭头标注「知识供给 · 运营反馈」。
- 配色沿用架构总图色系；cairosvg 渲染验证布局正常；仅新增配图，跳过 Claude 只读审查。

### 任务：知识应用场景全景图 v1.0 场景具象化（行为场景图标）

- 18 个场景 chip 各加行为场景小图标（居左、文字右移，按卡系配色）：问答咨询型——耳麦（智能客服）、文书（制度/政策）、?圆徽（FAQ）；门户浏览型——放大镜（全文检索）、?页（帮助中心）、双链框（词条互链）；学习赋能型——学位帽（培训路径）、铃铛（知识推送）、核对板（考核陪练）；任务执行型——机器人（数字员工办事）、票据（工单）、报告页+柱状（报告生成）；分析推理型——关系网（风险传导）、准星（根因定位）、双人（组织关系）；语义治理型——标签（打标）、地球+检索柄（跨域检索）、双向对齐箭头（概念对齐）。
- cairosvg 渲染验证正常；仅配图微调，跳过 Claude 只读审查。

### 任务：知识应用场景全景图 v1.0 补充电信运营商内部场景

- 六张场景卡各增加一行「运营商场景」（浅色底 + 卡系色条标识，与通用场景行区分）：问答咨询型——10086 客服话术助手 / 资费套餐政策问答 / 业务受理规则问答；门户浏览型——网络运维知识库 / 设备厂商手册库 / 营业厅业务手册；学习赋能型——网优认证考核陪练 / 营业员上岗培训 / 装维技能知识包；任务执行型——故障工单定位派单 / 割接方案自动生成 / 网优报告自动生成；分析推理型——告警根因定位 / 网元影响分析 / 合同 SLA 关联审查；语义治理型——网络资源目录打标 / 业务术语对齐 / 跨省术语统一。
- 副标题更新为「每卡第二行为电信运营商内部典型场景」；典型用户与底部技术说明行同步融入运营商角色（客服中心/营业厅、网络部/综调中心、workhub 数字员工平台等）。
- 修复卡 3 / 卡 6 运营商行与通用场景行 y 坐标重叠（696→716）；cairosvg 渲染验证正常；仅配图改动，跳过 Claude 只读审查。
